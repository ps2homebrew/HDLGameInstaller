...
static inline int DecompressRLEData(void *output, unsigned int OutputBufferLength, const void *PackedData)
{
    ...
    for (offset = 0, pInput = (short int *)PackedDataPayload, pOutput = (unsigned short int *)output; offset < PackedDataLength && offset < OutputBufferLength;) {
        ...
        offset += 2;
        if (BlockLength > 0) {
            for (i = 0; i < BlockLength; i++, pOutput++)
                *pOutput = *pInput;
            pInput++;
...
void UnloadPS2IconModel(struct PS2IconModel *IconModel)
{
    ...
    for (vertex = 0; vertex < IconModel->ModelSectionHeader.nbvtx; vertex++) {
        if (IconModel->ModelVertices[vertex].vtx != NULL)
            free(IconModel->ModelVertices[vertex].vtx);
    }
    if (IconModel->ModelVertices != NULL)
        free(IconModel->ModelVertices);
    for (sequence = 0; sequence < IconModel->ModelAnimSectionHeader.nbseq; sequence++) {
        for (i = 0; i < IconModel->ModelAnimSequences[sequence].nbksp; i++) {
            if (IconModel->ModelAnimSequences[sequence].frames[i].keys != NULL)
                free(IconModel->ModelAnimSequences[sequence].frames[i].keys);
        }
        if (IconModel->ModelAnimSequences[sequence].frames != NULL)
            free(IconModel->ModelAnimSequences[sequence].frames);
    }
    if (IconModel->ModelAnimSequences != NULL)
        free(IconModel->ModelAnimSequences);
    if (IconModel->texture != NULL)
        free(IconModel->texture);
}
