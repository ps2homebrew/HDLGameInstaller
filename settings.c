...
static int WriteSettings(void)
{
    ...
    if ((buffer = memalign(64, 512)) != NULL) {
        len = sprintf(buffer, "SORT = %s\n"
                      ...
                      WriteBoolean(RuntimeData.EthernetFlowControl)) +
              1;
    ...
    if ((result = fileXioMount("pfs1:", "hdd0:__common", FIO_MT_RDWR)) >= 0) {
        ...
        if (result == 0) {
            if ((fd = fileXioOpen(TITLE_SAVE_FOLDER_FULL "/icon.sys", O_RDONLY)) >= 0) {
                result = 0;
                fileXioClose(fd);
            } else { // Could not open. Probably not created.
        ...
        if (result == 0) {                             // Write the icon.
            if ((result = WriteSaveIcon()) == 0) {     // Write the settings file.
                if ((result = WriteSettings()) == 0) { // Write the icon.sys file last. User can identify the corrupt data and delete it, if writing failed along the way.
